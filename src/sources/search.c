...
void acc_init_from_board(AccumulatorPair *restrict acc_pair, const Board *restrict board) {
    ...
    acc_init(&acc_pair->pov[WHITE], &GlobalNetwork);
    acc_init(&acc_pair->pov[BLACK], &GlobalNetwork);
    ...
    for (Bitboard occ = board_occupancy_bb(board); occ;) {
        ...
        acc_add(&acc_pair->pov[WHITE], widx, &GlobalNetwork);
        acc_add(&acc_pair->pov[BLACK], bidx, &GlobalNetwork);
    }
}
...
void acc_update_once(AccumulatorPair *restrict cur_acc, const AccumulatorPair *restrict prev_acc, const Boardstack *restrict stack) {
    ...
    // Take care of null moves.
    if (stack->feature_add[0] == 0) {
        return;
    }

    acc_pair_add(cur_acc, stack->feature_add[0], &GlobalNetwork);
    acc_pair_sub(cur_acc, stack->feature_sub[0], &GlobalNetwork);

    if (stack->feature_add[1] != 0) {
        acc_pair_add(cur_acc, stack->feature_add[1], &GlobalNetwork);
    }

    if (stack->feature_sub[1] != 0) {
        acc_pair_sub(cur_acc, stack->feature_sub[1], &GlobalNetwork);
    }
}

void acc_update_r(Worker *worker, const Boardstack *restrict stack, Searchstack *ss) {
    if (ss->acc_up_to_date) {
        return;
    }

    assert(ss->plies > 0);
    acc_update_r(worker, stack->previous, ss - 1);
    acc_update_once(&worker->acc_stack[ss->plies], &worker->acc_stack[ss->plies - 1], stack);
    ss->acc_up_to_date = true;
}

void acc_update(Worker *worker, const Board *restrict board, Searchstack *ss) {
    acc_update_r(worker, board->stack, ss);
}
...
Score search(
    ...
    if (ss->plies >= MAX_PLIES) {
        return !board->stack->checkers ? evaluate_noacc(board) : worker_draw_score(worker);
    }
    ...
    // The root node always has up to date accumulators so that its children can depend on it.
    if (root_node) {
        acc_init_from_board(worker->acc_stack, board);
        ss->acc_up_to_date = true;
    } else {
        ss->acc_up_to_date = false;
    }
    ...
    // Call the evaluation function otherwise.
        ...
        extern Network GlobalNetwork;
        acc_update(worker, board, ss);
        raw_eval = nn_evaluate(board, &GlobalNetwork, &worker->acc_stack[ss->plies]);
