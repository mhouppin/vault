...
static const Command UciCommands[] = {
    ...
    {STATIC_STRVIEW("genfens"), uci_genfens},
...
void uci_d(Uci *uci, __attribute__((unused)) StringView args) {
    ...
    Score eval = nn_evaluate_noacc(&uci->root_board, &GlobalNetwork);
...
static void do_genfens(u64 count, u64 seed, u8 depth) {
    ...
    for (u64 i = 0; i < count; ++i) {
        ...
        board_try_init(&board, StartposStr, false, stack);
        ...
        for (d = 0; d < rdepth; ++d) {
            ...
            u64 r = u64_random(&state);
            Move move = list.moves[r % movelist_size(&list)].move;
...
void uci_genfens(Uci *uci __attribute__((unused)), StringView args) {
    ...
    if (!strview_parse_u64(token, &count)) {
        info_debug(
            "info string Error: invalid value for FEN count '%.*s'\n",
            (int)token.size,
    ...
    if (!strview_equals_strview(token, STATIC_STRVIEW("seed"))) {
        info_debug(
            "info string Error: expected command 'seed', got '%.*s'\n",
            (int)token.size,
            (const char *)token.data
        );
        return;
    ...
    if (!strview_parse_u64(token, &seed)) {
        info_debug(
            "info string Error: invalid value for seed '%.*s'\n",
            (int)token.size,
            (const char *)token.data
        );
        return;
